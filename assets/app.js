...
(function () {
  ...
  var DEFAULT_RULES = [
    ...
    { kw: '医院 药房 药店 诊所 挂号 体检 医保 牙科 门诊 住院 保健品 药�� 医药 诊所 化验', cat: 'c6' },
  ...
  function renderDetectBatch() {
    ...
    card.querySelector('#dc-batch-save').addEventListener('click', function () {
      detectPending.forEach(function (r) {
        var cat = autoClassify(r.remark, r.type) || defaultCat(r.type).id;
        state.bills.push({ id: uid(), type: r.type, amount: r.amount, cat: cat, remark: r.remark || '自动识别账单', date: r.date, source: 'auto' });
      });
      var n = detectPending.length;
      detectPending = [];
      save();
      card.className = 'detect-card';
      card.innerHTML = '';
  ...
  function startAutoDetect() {
    ...
    autoTimer = setInterval(function () {
      ...
      navigator.clipboard.readText().then(function (text) {
        ...
        var cat = autoClassify(res.remark, res.type) || defaultCat(res.type).id;
        state.bills.push({ id: uid(), type: res.type, amount: res.amount, cat: cat, remark: res.remark || '自动识别账单', date: res.date, source: 'auto' });
        save();
        renderHome();
        checkBudgetWarn();
        toast((res.type === 'income' ? '自动记录收入' : '自动记录支出') + ' ¥' + fmtMoney(res.amount) + ' · ' + (getCat(cat) || { name: '未分类' }).name);
  ...
  /* 语音识别：单句模式。iOS Safari 对 continuous + 自动重启不稳定。
     一次点击只识别一句，结果进入确认弹层，不直接写入账单。 */
  var voiceRec = null;
  ...
  function voiceUI(recording) {
    ...
    if (voiceHint) {
      if (recording) voiceHint.textContent = '🎙️ 请说一句账单，如「美团外卖25元」或「收到工资5000」';
  ...
  function isDuplicateVoice(text) {
    var normalized = String(text || '').replace(/\s+/g, '').trim();
    var now = Date.now();
    if (!normalized) return true;
    if (normalized === lastVoiceText && now - lastVoiceAt < 3000) return true;
  ...
  function startVoice(onResult, hintEl, btnEl) {
    ...
    voiceTimeout = setTimeout(function () {
      ...
      stopVoice();
      toast('没有识别到语音，请重新点击并说出金额');
    }, 12000);
    rec.onresult = function (e) {
      if (!voiceListening || !e.results || !e.results.length) return;
      var result = e.results[e.results.length - 1];
      var text = result && result[0] ? String(result[0].transcript || '').trim() : '';
      if (!text || isDuplicateVoice(text)) return;
      clearVoiceTimeout();
      if (voiceHint) voiceHint.textContent = '识别到：「' + text + '」';
      stopVoice();
      if (voiceOnResult) voiceOnResult(text);
    };
    rec.onerror = function (e) {
      ...
      var error = e && e.error;
      stopVoice();
      if (error === 'not-allowed' || error === 'service-not-allowed') toast('请允许麦克风权限：设置 → Safari → 麦克风');
      else if (error === 'no-speech') toast('没有听到语音，请靠近麦克风再试');
      else if (error === 'audio-capture') toast('麦克风不可用，请关闭占用麦克风的应用后重试');
      else if (error === 'network') toast('语音服务连接失败，请检查网络后重试');
      else if (error !== 'aborted') toast('语音识别失败，请重试');
    };
    rec.onend = function () {
      /* 单句模式自然结束时释放状态，不自动 start，避免 iOS Safari 假死。 */
      if (voiceListening) {
        clearVoiceTimeout();
        voiceListening = false;
        voiceRec = null;
        voiceUI(false);
        toast('没有识别到完整语句，请再试一次');
      }
    };
    try { rec.start(); } catch (e) { stopVoice(); toast('语音识别启动失败，请稍后重试'); }
  }
  /* 页签切换时自动停止语音，释放资源 */
  ...
  function handleLaunchParams() {
    try {
      ...
      if (params.auto === '1' && params.add) {
        ...
        if (r && r.amount !== null) {
          var cat = autoClassify(r.remark, r.type) || defaultCat(r.type).id;
          state.bills.push({ id: uid(), type: r.type, amount: r.amount, cat: cat, remark: r.remark || '快捷记账', date: r.date, source: 'shortcut' });
          save();
          renderHome();
          toast('已快捷入账 ¥' + fmtMoney(r.amount) + ' · ' + (getCat(cat) || { name: '未分类' }).name);
        } else {
          toast('未能识别金额，请检查内容');
        }
        return;
